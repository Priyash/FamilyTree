...
public class FetchBrotherInLaw implements Fetchable {
    ...
    //SINGLE FUNCTION FOR FINDING THE BROTHER IN LAWS OVERALL COMBINING THE TWO LOGIC
    ...
    private LinkedList<IFamilyMember> getBrotherInLaws_2(IFamilyMember familyMember, IFamilyMember familyMemberToFind) {
        ...
        for (Map.Entry<IFamilyMember, IRelationship> entry : children.entrySet()) {
            if (entry.getKey().getMemberName().equals(familyMemberToFind.getMemberName()) && FAMILY_MEMBER_FOUND == false) {
    ...
    //THIS FUNCTION CHECKS A DAUGHTER BELONG TO A TREE HAS A HUSBAND OR NOT [GIVEN HUSBAND NAME IN THE QUERY FOR BROTHER-IN-LAW SEARCH]
    private Boolean hasHusband(IFamilyMember member, IFamilyMember familyMemberToFind)
    {
        ...
        for (Map.Entry<IFamilyMember, IRelationship> entry : children.entrySet()) {
            if(entry.getValue().getRelationType().equals(Constants.HUSBAND) && entry.getKey().getMemberName().equals(familyMemberToFind.getMemberName()) && FAMILY_MEMBER_FOUND == false)
            {
                FAMILY_MEMBER_FOUND = true;
                hasHusband = true;
                break;
    ...
    // THIS FUNCTION CHECKS A DAUGHTER BELONG TO A TREE HAS A HUSBAND OR NOT[OVERLOADED FUNCTION FOR ANOTHER LOGIC]
